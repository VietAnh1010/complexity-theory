...
Machine-checked complexity bounds for **BigO(Bench)** solutions
...
annotation. A curve fit cannot be verified. So: model a solution, prove a
bound, check whether the label agrees. **The disagreements are the output.**

## Cost model convention

**Input and output are not charged.** Cost counts the work a solution does
once its input is available; reading stdin, parsing a line, and printing are
free. Fixed in `theories/Cost.v`, restated in every example header.

Every program that reads its input is Ω(input size), so charging the read makes
that trivial fact the finding whenever the real work is smaller. That is what
happened to `450_204`, now withdrawn — see Entries.
...
`make check` fails on a build error, on any `Admitted`, and — via
`tools/check_axioms.sh` — if a headline theorem depends on an axiom.
...
Six. Selection method is recorded per row in `samples/manifest.jsonl`.
...
| `603_284` | 1041_A Heist | `O(nlogn)` | `n−1 ≤ cost ≤ 2n·log₂⌈n⌉ + n` | SoundUpperOnly | `Examples/S603_284.v` |
| `1421_53` | p02899 ABC 142 Go to School | `O(n+m)` | `cost = n` | SoundLoose | `Examples/S1421_53.v` |
| `433_57` | 257_B Playing Cubes | `O(1)` | `cost = 3` on every input | Tight | `Examples/S433_57.v` |
| `5_100` | 622_A Infinite Sequence | `O(n)` | `Θ(√n)`, and ≥ 2^k on 2k+3 bits | Overstated | `Examples/S5_100.v` |
| `167_177` | 1184_A1 Heidi Learns Hashing | `O(n)` | `cost ≤ √n` | Overstated | `Examples/S167_177.v` |

**Withdrawn: `450_204`** (1421_C Palindromifier, fitted `O(1)`). Recorded as
the set's only wrong label, on a proof that charged `input()` one tick per
character. That makes any program reading its input Ω(input size), so it said
nothing about this solution, which does O(1) work. Under the convention above
the fitted `O(1)` is correct. `Examples/S450_204.v` is kept and still compiles:
it proves the constant-cost result and, separately, what the withdrawn verdict
rested on. It has no `Catalog.v` row. `433_57` replaces it.
...
**No entry is `NotUpperBound`.** All six labels are sound upper bounds; two are
`Overstated`, a precision failure rather than an error — O(n) does bound a √n
loop, it just says nothing useful. The one former `NotUpperBound` was withdrawn
as an artifact of charging input.
...
python3 tools/bench.py sample --n 30 --seed 20260817
python3 tools/bench.py scaffold 5_100
```

Use `sample`, not `list`, for anything meant to support a rate: `list` sorts by
length.
...
**2. Model it in the cost monad.** `M A` pairs a value with a step count; `tick`
charges one, `charge n` charges `n`, `<- ;;` sequences and adds. The algorithm
and its cost are one definition, so cost cannot drift from code — a
...
- **What counts as work.** Input and output are free by convention. That is
  the standard reading and it is what `Cost.v` fixes, but it is a choice, and
  it decided `450_204`: charged, that program is Θ(n); free, it is O(1).
...
- **What "n" means.** In `S5_100.v` and `S167_177.v` it is an integer's *value*,
  not a length. `S5_100.v` proves both readings: `iters_sqrt` gives ~√(2n) in
  the value, `iters_exponential_in_bitlength` gives ≥ 2^k iterations on a
  (2k+3)-bit input. Under one convention the loop beats the fitted O(n); under
  the other it is not polynomial at all. The framework takes sizes from the
  dataclass fields, so these files follow that — not the textbook convention.
...
`Alt/Deep.v` is a small imperative language with a step-counting big-step
semantics. `Alt/S2389_139_Deep.v` and `Alt/S5_100_Deep.v` re-model two solutions
in it, sharing no definitions with the monad versions and reaching the same
bounds.
...
- Relational, so no fuel and no termination obligation — a diverging program has
  no derivation. Compare `S5_100.v`'s `run_finishes`.
- Cost theorems quantify over *every* derivation, so no determinism lemma is
  needed to rule out a cheaper one.
- `SWhile` is in the language, so `Alt/S5_100_Deep.v` re-proves the √n bound
  with **no fuel and no adequacy lemma** — `S5_100.v` needs `run_finishes` only
  because a fuel-truncated loop satisfies any upper bound for free.
- The price: the semantics is not total, so `prog_sqrt` is conditional on a
  derivation existing. `run_4` is a worked witness, which keeps the theorem from
  being vacuous.
- Cost: ~175 lines of language + ~130 per example, against 78 for the monad
  version.

Agreement between the two styles is a sanity check, not a requirement: recorded
where it falls out, not forced where it does not. It would not validate the cost
model anyway — both styles encode the same judgement about what `str.count`
costs, so a wrong reading of Python is wrong in both.
...
- **No rate is claimed.** Six entries say nothing about the 640-record test
  set, and selection was not uniform over it.
- **No label in the set is wrong.** The one that looked wrong was a cost-model
  artifact. What remains is two `Overstated` labels and one spurious size
  parameter — precision failures, not errors.
- Both `Overstated` verdicts are √n loops labelled O(n), reached by different
  ...
  blind spot, but two cases is not evidence of one.
- `603_284` has an open gap between its proved lower and upper bounds, recorded
  rather than closed with a model artefact. Its Ω(n) now comes from the pairwise
  scan; before the convention change it came from the input parse — the same
  artifact that sank `450_204`.
- Time test set only. Space is untouched: the monad counts steps and would need
  a separate high-water-mark measure.
