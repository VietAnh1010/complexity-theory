...
//
//   evidence:
//     dArr is declared as seq<int> of length b and is written with `dArr
//     := dArr[q3 := ...]` or `dArr[q4 := ...]` inside the while loop over
//     c_list, and each such seq index-update copies all b elements, so a
//     loop of length n performing up to n such updates costs O(n*b) =
//     O(n*m); Python's equivalent `d[q4] += 1` mutates a list element in
//     place in O(1), matching the label O(n) that Dafny no longer
//     achieves.
//
//   structural facts (deterministic, from labelaudit.py):
//     {"body_lines": 82, "data_dependent_loops": 0, "decreases_star":
//     false, "linear_prelude_calls": ["JoinInts"], "loop_depth": 1,
//     "loops": 2, "recursive_helpers": 2, "seq_append_read_in_same_loop":
