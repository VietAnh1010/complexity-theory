...
method T(b0In: int, b1In: int, prIn: int) returns (p0: int, p1: int, ghost steps: nat)
  requires b0In >= 0 && b1In >= 0
  ensures steps <= 6 * (b0In + b1In) + 1
{
  steps := 1;
  var b0 := b0In;
  var b1 := b1In;
  ...
  while b0 > 0 || b1 > 0
    invariant b0 >= 0 && b1 >= 0
    invariant steps <= base1 + 6 * ((b0In + b1In) - (b0 + b1))
    decreases b0 + b1
  {
    var c: int;
    if turn == 0 {
