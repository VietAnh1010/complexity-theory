...
lemma CountOnesUpToLe(s: seq<int>, k: nat)
  requires k <= |s|
  ensures CountOnesUpTo(s, k) <= k
  decreases k
{
  if k > 0 {
    CountOnesUpToLe(s, k - 1);
  }
}
  ...
  ensures steps <= 14 * |c_list| + 15
{
  ...
  while idx < |c_list|
    ...
    invariant q4 <= |f|
    invariant steps <= afterFirst + 6 * idx
