...
// Label O(n). The first loop is a binary-search style narrowing of [i, j):
// each iteration strictly shrinks j - i, so it runs at most n iterations even
// though it usually finishes in O(log n). The second loop runs cr <= n - 1
