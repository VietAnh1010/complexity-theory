...
method Solve(v_0: seq<string>) returns (output: string, ghost steps: nat)
  ensures steps <= 10 * (if |v_0| >= 2 then |v_0[0]| + |v_0[1]| else 0) + 30
{
  steps := 1;
  if |v_0| < 2 {
    output := "";
  ...
  if result == 0 {
    ...
    output := l0b + "+" + l1b + "=" + l2b;
    steps := steps + |l0b| + |l1b| + |l2b| + 10;
  } else if result == -2 {
    var l0b := l0 + "|";
    var l2b := if |l2| > 0 then l2[..|l2|-1] else l2;
    output := l0b + "+" + l1 + "=" + l2b;
    steps := steps + |l0b| + |l1| + |l2b| + 10;
  } else {
    output := "Impossible";
    steps := steps + 1;
  }
