...
method Solve(n: int, board: seq<string>) returns (output: string, ghost steps: nat)
  requires n >= 0
  ensures steps <= 6 * n * n + n + 3
{
  steps := 1;
  var ok := true;
  var i := 1;
  while i <= n
    invariant 1 <= i <= n + 1
    invariant steps == 1 + (i - 1) * (6 * n + 1)
    decreases n - i
  {
    var j := 1;
    while j <= n
      invariant 1 <= j <= n + 1
      invariant steps == 1 + (i - 1) * (6 * n + 1) + (j - 1) * 6
      decreases n - j
    {
