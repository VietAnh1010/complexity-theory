...
// SortInts([cntA,cntB,cntC]) sorts a fixed-length-3 sequence -- O(1). Each of
// SolveOne's two scans is linear in |a|; Solve's outer loop sums those over
// `strings`, giving the honest additive bound over the total input length.
...
method SolveOne(a: string) returns (ok: bool, ghost steps: nat)
  ensures steps <= 4 * |a| + 30
{
  steps := 1;
  var m := |a|;
  var cntA := 0;
  ...
  ensures steps <= 4 * SumLen(strings) + 34 * |strings| + 3
{
  ...
  while i < |strings|
    ...
    invariant steps <= base1 + (4 * SumLen(strings[..i]) + 34 * i)
    decreases |strings| - i
  {
    var ok, oneSteps := SolveOne(strings[i]);
    assert strings[..i+1] == strings[..i] + [strings[i]];
    SumLenSnoc(strings[..i], strings[i]);
    parts := parts + [if ok then "YES\n" else "NO\n"];
    i := i + 1;
    steps := steps + oneSteps + 2;
  }
