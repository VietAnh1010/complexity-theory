// 1108_A. Two distinct points  (problem 936, solution 936_772)
// time complexity: O(n)
// python exact-diff baseline: exact
//
// Reproduce the Python program's entire stdout in `output`.
//
// --- Python ---------------------------------------------------------
// q = int(input())
// for i in range(q):
//     l1,r1,l2,r2 = input().split()
//     l1=int(l1)
//     l2=int(l2)
//     r1=int(r1)
//     r2=int(r2)
//     if l1 != l2:
//         print (l1,l2)
//     elif l1 != r2:
//         print (l1,r2)
//     elif r1 != l2:
//         print (r1,l2)
//     elif r1 != r2:
//         print (r1,r2)
// --------------------------------------------------------------------

include "../../prelude.dfy"
import opened Prelude

// Every "output + t" here has t bounded by the digit-string exception
// (IntToString and its length are both charged 1) or a short literal, so
// each iteration is charged a constant, independent of how long `output`
// has already grown.
method Solve(n: int, grid: seq<seq<int>>) returns (output: string, ghost steps: nat)
  requires forall k :: 0 <= k < |grid| ==> |grid[k]| >= 4
  ensures steps <= 5 * |grid| + 1
{
  output := "";
  var i := 0;
  steps := 1;
  while i < |grid|
    invariant 0 <= i <= |grid|
    invariant steps <= 1 + 5 * i
    decreases |grid| - i
  {
    var row := grid[i];
    var l1 := row[0];
    var r1 := row[1];
    var l2 := row[2];
    var r2 := row[3];
    if l1 != l2 {
      output := output + IntToString(l1) + " " + IntToString(l2) + "\n";
    } else if l1 != r2 {
      output := output + IntToString(l1) + " " + IntToString(r2) + "\n";
    } else if r1 != l2 {
      output := output + IntToString(r1) + " " + IntToString(l2) + "\n";
    } else if r1 != r2 {
      output := output + IntToString(r1) + " " + IntToString(r2) + "\n";
    }
    i := i + 1;
    steps := steps + 5;
  }
}
