...
5. **If your proved bound disagrees with the label, keep the proof and record
   the disagreement.** Never weaken a proof to match a label. The label is a
   regression over profiling runs; a proof holds for every input. A disagreement
   is a finding and is worth more than a success.
...
Append one JSON object per row to **your own** `traj_<SLICE>.jsonl` in this
directory. Never write to another agent's file.

```json
{"solution_id":"1053_38","label":"O(n)","outcome":"proved",
 "bound":"3 * |a_list| + 4","agrees_with_label":true,
...
multiplication the solver would not do. Not "ran out of attempts".

## Reporting

Be concise. No preamble, no per-command narration. Final message ≤ 8 lines:
counts proved / unresolved, any bound that disagrees with its label, and the
