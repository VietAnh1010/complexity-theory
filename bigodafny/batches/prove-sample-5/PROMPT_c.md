...
Your rows are the ones listed in `batches/prove-sample-5/slice_c.jsonl`. Read that
...
## The prelude can now prove its sort is ordered

Added 2026-09-22. If your bound needs "the last element after sorting is the
largest" -- a loop running to `sorted[|sorted|-1]`, say -- you no longer have
to give up:

```dafny
IntLessIsTotalOrder();       // discharges StrictTotalOrder for int's <
SortIntsIsSorted(s);         // IsSorted, length preserved, last >= every x
```

For any other comparator, prove `StrictTotalOrder(less)` -- irreflexivity,
transitivity, totality -- then call `SortIsSorted(s, less)` or
`SortLastIsMax(s, less)`. Passing the comparator as a lemma PARAMETER matters:
a ghost function returning a lambda gives Dafny a value it will not identify
with the same lambda at the call site, and every fact about `Sort` then fails
to transfer.

`solutions-proved/value-bounded/2423/2423_48.dfy` is a worked example.
...
Append one JSON object per row to **your own** `batches/prove-sample-5/traj_c.jsonl`.
