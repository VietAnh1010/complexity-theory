...
> Moved from a top-level `solution-guessed-verified/`. It is an experiment
> output, written by `experiments/archive_attempts.py`, and it holds no dataset
> rows — it does not belong beside the `solutions-*` directories, which do.

The name is the contrast with `solutions-proved/`:

| directory | the label was |
|---|---|
|  `solutions-proved/` | **known** to whoever wrote the proof |
| `experiments/proofs-blind/` | **withheld**; the agent committed to a class in writing first |
