...
| operation | real cost |
|---|---|
| `s[i := v]` (seq functional update) | `\|s\|` -- measured, a full copy |
| `a[i] := v` on an `array<T>` | `1` -- measured, in place |
...
**A seq UPDATE is not an append.** `s := s[i := v]` copies the whole sequence
every time -- there is no laziness here, and reading or not reading makes no
difference. In a loop over `n` elements that is quadratic, and it is quadratic
in the compiled program, not only in the charge:

    s := s[i := v]   n=2k .070s  4k .144s  8k .471s  16k 1.735s
    a[i] := v        n=2k .036s  4k .038s  8k .037s  16k .042s

If a method's cost is dominated by a seq update in a loop, that is a real
quadratic and saying so is the right answer, not a proof failure.
...
**`Join` is linear**: charge it `SumLen(parts) + |parts|`, the total output
length plus one per part. An earlier version of this guide said it had no
honest charge and told you to give up on rows dominated by it. That was wrong
-- ratios of ~2.2 per doubling were read as an exponent when a known-linear
control in the same harness gives 2.40/2.32/2.24. Per-line output is ordinary
and provable.
